...
function setupDrawingPad(canvasId, loadImageData = null) {
    ...
    const resizeCanvas = () => {
        ...
        if (loadImageData && loadImageData !== '') {
            ...
            // ölçeklendirerek eski içeriği korumaya çalış
...
function saveHistoryState(canvasId) {
    ...
    // Geçmişteki ileri dosyaları temizle
    if (state.historyIndex < state.history.length - 1) {
        state.history = state.history.slice(0, state.historyIndex + 1);
    }
    state.history.push(imageData);
    state.historyIndex = state.history.length - 1;
    // Aşırı büyümesin (son 50)
...
function updateBrushColor(canvasId) {
    ...
    const ctx = state.ctx;
    if (state.isErasing) {
        ctx.strokeStyle = '#fefefe'; // silgi (beyaz)
    } else {
        ctx.strokeStyle = '#1a1a2e'; // siyah kalem
    }
...
function loadState() {
    try {
        ...
        if (saved.version === 7.0 || saved.version === 7.1 || saved.version === 7.2) {
...
function startApp() {
    ...
    console.log('✅ Uygulama hazır! (Çizim + Silgi + Geri Al)');
}
