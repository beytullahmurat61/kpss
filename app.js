...
// Versiyon: 5.1 - HATASIZ
// ============================================

console.log('🚀 app.js yükleniyor...');

// ============================================
// SADECE BURADA TANIMLA (LEVELS YOK!)
// ============================================
const STATE_VERSION = 5.1;

// ============================================
// STATE YÖNETİMİ (SADECE BİR KEZ)
// ============================================
let ST = {
...
// ============================================
// TOPİK YARDIMCILARI (LEVELS config.js'DEN GELİR)
// ============================================
...
function convertQuestionBankToTemplates() {
    ...
    for (let topicId = 1; topicId <= 205; topicId++) QUESTION_TEMPLATES[topicId] = [];
    for (let level in SORU_BANKASI) {
        const levelNum = parseInt(level);
        const startTopicId = levelNum * 10 + 1;
        const endTopicId = Math.min(startTopicId + 10, 205);
        for (let template of SORU_BANKASI[level]) {
            for (let topicId = startTopicId; topicId <= endTopicId; topicId++) {
                if (QUESTION_TEMPLATES[topicId]) QUESTION_TEMPLATES[topicId].push({ ...template });
            }
        }
    }
...
function generateQuestion(topicId, level) {
    if (!QUESTION_TEMPLATES[topicId] || !QUESTION_TEMPLATES[topicId].length) {
        return { id: 'fallback', soru: '1 + 1 = ?', cevap: '2', cevapRaw: 2, zorluk: 'kolay' };
    }
...
function loadState() {
    try {
        const saved = JSON.parse(localStorage.getItem('kpss_mat_v4') || '{}');
        if (saved.version === STATE_VERSION) Object.assign(ST, saved);
    } catch (e) { console.warn(e); }
    ST.apiKey = localStorage.getItem('kpss_mat_api_key') || '';
    for (let i = 1; i <= 205; i++) if (!ST.hist[i]) ST.hist[i] = { levels: {}, currentLevel: 'KOLAY' };
...
function saveState() {
    try {
        ...
        localStorage.setItem('kpss_mat_v4', JSON.stringify(d));
    } catch (e) { console.warn(e); }
}
...
function renderTopicsList() {
    ...
    TOPICS.forEach(t => {
        ...
        html += `<div class="${cls}" ${locked ? '' : `onclick="openTopic(${t.id})"`}><span class="t-icon">${t.e}</span><div class="t-info"><div class="t-name">${t.n}</div><div class="prog-bar-wrap"><div class="prog-bar-bg"><div class="prog-bar-fill fill-acc" style="width:${pct}%"></div></div></div></div><span>${icon}</span></div>`;
    });
    el.innerHTML = html;
}
...
window.checkAnswer = function() {
    ...
    if (hist.levels[level].total >= limit) {
        ...
        } else { hist.levels[level].correct = 0; hist.levels[level].total = 0; }
    }
    saveState();
    ST.phase = 'feedback';
    let nextMsg = '';
    ...
    const el = document.getElementById('learnContent');
    el.innerHTML += `<div class="fb ${isCorrect ? 'fb-ok' : 'fb-fail'}"><div class="fb-head"><span class="fb-icon">${isCorrect ? '🎉' : '❌'}</span><span class="fb-title">${isCorrect ? 'Doğru!' : 'Yanlış'}</span></div><div class="fb-body">${isCorrect ? `Cevap: <strong>${ST.cq.cevap}</strong>` : `Doğru cevap: <strong>${ST.cq.cevap}</strong>`}</div>${nextMsg}${!topicCompleted ? '<div class="btn-row" style="margin-top:12px"><button class="btn btn-ghost btn-full" onclick="nextQuestion()">Sonraki Soru →</button></div>' : ''}</div>`;
};

window.nextQuestion = function() { ST.phase = 'question'; ST.cq = null; renderNextQuestion(); };

// ============================================
// SORU BANKASI LİSTESİ
// ============================================
...
function renderQuestionBankList() {
    ...
    TOPICS.forEach(t => {
        ...
        const done = s >= 300;
        html += `<div class="topic-row ${done ? 't-done' : ''}" onclick="startQuestionBank(${t.id})"><span class="t-icon">${t.e}</span><div class="t-info"><div class="t-name">${t.n}</div><div class="t-meta">${done ? '✅ Tamamlandı' : `${s}/300`}</div><div class="prog-bar-wrap"><div class="prog-bar-bg"><div class="prog-bar-fill fill-acc" style="width:${pct}%"></div></div></div></div><span>${done ? '✅' : '📝'}</span></div>`;
    });
    el.innerHTML = html;
}
...
window.checkQBAnswer = function() {
    ...
    renderQBSolveHeader();
    const el = document.getElementById('qbSolveContent');
    el.innerHTML += `<div class="fb ${isCorrect ? 'fb-ok' : 'fb-fail'}"><div class="fb-head"><span class="fb-icon">${isCorrect ? '🎉' : '❌'}</span><span class="fb-title">${isCorrect ? 'Doğru!' : 'Yanlış'}</span></div><div class="fb-body">${isCorrect ? `Cevap: <strong>${ST.cq.cevap}</strong>` : `Doğru: <strong>${ST.cq.cevap}</strong>`}</div><div class="btn-row" style="margin-top:12px"><button class="btn btn-ghost btn-full" onclick="nextQBQuestion()">Sonraki →</button></div></div>`;
};
...
function initExamSets() {
    for (let i = 1; i <= 5; i++) {
        if (!ST.examSets['set_' + i]) ST.examSets['set_' + i] = { completed: false, net: null, date: null };
    }
    saveState();
}
...
window.startExamSet = function(setId) {
    ...
    ST.currentExam = { setId, questions: examQuestions, currentIndex: 0, answers: [], timeLeft: EXAM_DURATION * 60 };
    showView('vExam');
...
function finishExam() {
    ...
    saveState();
    const el = document.getElementById('examContent');
    el.innerHTML = `<div style="text-align:center;padding:20px 0"><div class="net-num">${net}</div><div class="net-lbl">Net</div><div class="stat-grid"><div class="stat-cell"><div class="stat-num" style="color:var(--success)">${d}</div><div class="stat-lbl">Doğru</div></div><div class="stat-cell"><div class="stat-num" style="color:var(--danger)">${y}</div><div class="stat-lbl">Yanlış</div></div><div class="stat-cell"><div class="stat-num" style="color:var(--warning)">${answers.length}</div><div class="stat-lbl">Toplam</div></div></div><div class="btn-row"><button class="btn btn-primary btn-full" onclick="startExamSet('${ST.currentExam.setId}')">🔄 Tekrar</button><button class="btn btn-ghost btn-full" onclick="goExamList()">Listeye Dön</button></div></div>`;
    ST.currentExam = null;
}
...
function renderStats() {
    ...
    let qbs = 0;
    Object.keys(ST.questionBankProgress).forEach(tid => { qbs += ST.questionBankProgress[tid].solved.length; });
    el.innerHTML = `<div class="net-box"><div class="net-num">${Math.min(30, Math.round(done * 0.55 + acc / 14))}</div><div class="net-lbl">Tahmini KPSS Netin</div></div><div class="stat-grid"><div class="stat-cell"><div class="stat-num" style="color:var(--accent)">${done}</div><div class="stat-lbl">Konu Bitti</div></div><div class="stat-cell"><div class="stat-num" style="color:var(--danger)">${ST.totalQ}</div><div class="stat-lbl">Toplam Soru</div></div><div class="stat-cell"><div class="stat-num" style="color:var(--success)">%${acc}</div><div class="stat-lbl">Doğruluk</div></div><div class="stat-cell"><div class="stat-num" style="color:var(--warning)">${ST.maxStreak}</div><div class="stat-lbl">En İyi Seri</div></div></div><div class="card"><h3>Genel İlerleme</h3><div class="prog-bar-wrap"><div class="prog-bar-label"><span>Konular</span><span>${done}/205</span></div><div class="prog-bar-bg"><div class="prog-bar-fill fill-acc" style="width:${Math.round((done / 205) * 100)}%"></div></div></div><div class="prog-bar-wrap"><div class="prog-bar-label"><span>Soru Bankası</span><span>${qbs}/${205 * 300}</span></div><div class="prog-bar-bg"><div class="prog-bar-fill fill-grn" style="width:${Math.round((qbs / (205 * 300)) * 100)}%"></div></div></div></div>`;
}
...
window.doReset = function(type) {
    ...
    if (type === 'all') {
        ...
        for (let i = 1; i <= 205; i++) ST.hist[i] = { levels: {}, currentLevel: 'KOLAY' };
        initExamSets();
