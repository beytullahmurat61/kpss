...
function convertQuestionBankToTemplates() {
    ...
    if (typeof LEVEL_TOPIC_MAP === 'undefined') {
        console.warn('LEVEL_TOPIC_MAP tanımlı değil!');
        return;
    }

    // Tüm topic slotlarını başlat
    for (let topicId = 1; topicId <= 83; topicId++) {
        QUESTION_TEMPLATES[topicId] = [];
    }

    // Her questions.js level'ını ilgili topic ID'lerine dağıt
    for (let level in SORU_BANKASI) {
        const levelNum = parseInt(level);
        const mapping = LEVEL_TOPIC_MAP[levelNum];
        if (!mapping) continue;

        const templates = SORU_BANKASI[level];
        const topicIds = mapping.ids;

        // Bu level'daki soruları, ilgili tüm konulara eşit dağıt
    ...
    // Boş kalan konular için fallback: aynı level'daki ilk dolu konudan al
    for (let topicId = 1; topicId <= 83; topicId++) {
        if (QUESTION_TEMPLATES[topicId].length === 0) {
            // Aynı level'dan soru bul
            const topic = (typeof TOPICS !== 'undefined') ? TOPICS.find(t => t.id === topicId) : null;
            if (topic && typeof LEVEL_TOPIC_MAP !== 'undefined') {
                const lvl = topic.level;
                const siblingIds = LEVEL_TOPIC_MAP[lvl]?.ids || [];
                for (let sib of siblingIds) {
                    if (sib !== topicId && QUESTION_TEMPLATES[sib]?.length > 0) {
                        QUESTION_TEMPLATES[topicId] = [...QUESTION_TEMPLATES[sib]];
                        break;
                    }
                }
            }
    ...
    for (let i = 1; i <= 83; i++) {
        if (QUESTION_TEMPLATES[i] && QUESTION_TEMPLATES[i].length > 0) covered++;
        total += (QUESTION_TEMPLATES[i] || []).length;
    }
    console.log(`✅ Soru şablonları hazır. ${covered}/83 konu kapsandı. Toplam: ${total} şablon.`);
}
...
window.doReset = function(type) {
    ...
    if (type === 'all') {
        ...
        for (let i = 1; i <= 83; i++) ST.hist[i] = { levels: {}, currentLevel: '0' };
        initExamSets();
