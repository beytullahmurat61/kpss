...
// Müsvedde: Parmak/Kalem ile çizim alanı
// ============================================
...
let ST = {
    ...
    examHistory: [],
    scratchpad: '',          // artık base64 resim verisi tutacak
    phase: 'summary',
...
function shuffleArray(arr) { /* aynı */ const s = [...arr]; for (let i = s.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [s[i], s[j]] = [s[j], s[i]]; } return s; }
function todayStr() { return new Date().toISOString().split('T')[0]; }
function normAns(s) { /* aynı */ if (!s) return ''; let cleaned = String(s).toLowerCase().trim(); cleaned = cleaned.replace(/(\d+(?:\.\d+)?)\s*(?:tl|lira|gün|saat|km|kg|gr|lt|ml|cm|m)$/i, '$1'); cleaned = cleaned.replace(/,/g, '.').replace(/[×x]/g, '*').replace(/\s+/g, ''); return cleaned; }
function checkEqual(userAns, correctAns) { /* aynı */ try { const u = normAns(userAns), c = normAns(correctAns); if (u === c) return true; const uNum = parseFloat(u), cNum = parseFloat(c); if (!isNaN(uNum) && !isNaN(cNum) && Math.abs(uNum - cNum) < 0.001) return true; const uParts = u.split('/'), cParts = c.split('/'); if (cParts.length === 2 || uParts.length === 2) { const uVal = uParts.length === 2 ? Number(uParts[0])/Number(uParts[1]) : uNum; const cVal = cParts.length === 2 ? Number(cParts[0])/Number(cParts[1]) : cNum; if (!isNaN(uVal) && !isNaN(cVal) && Math.abs(uVal - cVal) < 0.001) return true; } return false; } catch(e) { return false; } }

// ========== GRAFİK YARDIMCILARI (aynen) ==========
...
function loadQuestions() { /* aynı */ for (let topicId = 1; topicId <= 20; topicId++) { QUESTION_TEMPLATES[topicId] = {}; for (let level = 0; level <= 2; level++) { QUESTION_TEMPLATES[topicId][level] = []; } } for (let topicId in SORU_BANKASI) { const tId = parseInt(topicId); for (let level in SORU_BANKASI[topicId]) { const lvl = parseInt(level); if (QUESTION_TEMPLATES[tId] && QUESTION_TEMPLATES[tId][lvl]) { QUESTION_TEMPLATES[tId][lvl] = SORU_BANKASI[topicId][level]; } } } console.log('✅ Sorular yüklendi'); }
function generateVariables(varRanges) { /* aynı */ if (!varRanges || Object.keys(varRanges).length === 0) return {}; const vars = {}; for (let [key, range] of Object.entries(varRanges)) { if (key === 'kosul') continue; if (Array.isArray(range)) { let [min, max, constraint] = range; min = Number(min); max = Number(max); let val = Math.floor(Math.random() * (max - min + 1)) + min; if (constraint === 'tek' && val % 2 === 0) val = val > min ? val - 1 : val + 1; else if (constraint === 'cift' && val % 2 !== 0) val = val > min ? val - 1 : val + 1; else if (typeof constraint === 'string' && constraint.endsWith('kati')) { const k = parseInt(constraint); if (!isNaN(k) && k > 0) val = Math.round(val / k) * k || k; } vars[key] = val; } else if (typeof range === 'string') { vars[key] = range; } else { vars[key] = range; } } return vars; }
...
function toggleDrawingPadSize(canvasId) {
    ...
    // Yeniden boyutlandır ve içeriği koru
    const rect = canvas.getBoundingClientRect();
...
// ========== STATE YÖNETİMİ (güncellendi) ==========
function loadState() {
    try {
        const saved = JSON.parse(localStorage.getItem('kpss_mat_v7') || '{}');
        if (saved.version === 7.0 || saved.version === 7.1) {
    ...
    // Eski text müsvedde varsa temizle (base64 değilse)
...
// ========== MÜSVEDDE (eski fonksiyonlar artık yeni canvas versiyonlarını çağırır, kullanılmıyor ama hata vermesin diye) ==========
...
function startApp() {
    ...
    console.log('✅ Uygulama hazır! (Çizim destekli müsvedde)');
}
