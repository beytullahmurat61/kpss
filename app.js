...
function generateVariables(varRanges) {
    ...
    for (let [key, range] of Object.entries(varRanges)) {
        ...
        if (Array.isArray(range)) {
            ...
            if (constraint === 'tek' && val % 2 === 0) val = val > min ? val - 1 : val + 1;
            else if (constraint === 'cift' && val % 2 !== 0) val = val > min ? val - 1 : val + 1;
            else if (typeof constraint === 'string' && constraint.endsWith('kati')) {
                const k = parseInt(constraint);
                if (!isNaN(k) && k > 0) val = Math.round(val / k) * k || k;
            }
            vars[key] = val;
